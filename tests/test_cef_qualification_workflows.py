...
class StrictQualificationWorkflowTests(unittest.TestCase):
    ...
    def test_strict_engine_iteration_uploads_only_encrypted_checkpoint(self):
        ...
        self.assertIn('LEGACY_CHECKPOINT_IMAGE_BY_RUN',worker)
        self.assertIn('REVIEWED_LEGACY_IMAGE_MIGRATIONS',worker)
        self.assertIn('critical_host_fingerprint',worker)
        self.assertIn('"critical_host_fingerprint"',worker)
        self.assertIn('"checkpoint_image_identity"',worker)
    ...
    def test_combined_qualification_runs_only_for_explicit_checkpoint_lock(self):
        ...
        self.assertIn('cef_native_link_static.prepare_restore(',worker)
        self.assertIn('cef_native_link_static.install(',worker)
        self.assertIn('cef_unwind_backtrace.install(',worker)
        self.assertIn('cef_x11_static.install(',worker)
        self.assertIn('cef_x11_static.audit_native(',worker)
        self.assertIn('source_fresh_binary_args()',worker)
        self.assertNotIn('reviewed_cache_args(',worker)
        self.assertIn('recipe = recipe_checkout',worker)
        self.assertIn('CEF_NSS_SHA256_Update',worker)
        self.assertIn('CEF_GTK_CODEC_jpeg_std_error',worker)
        self.assertIn('CEF_GTK_CODEC_TIFFOpen',worker)
        self.assertIn('relocated_engine_isolation_symbols_verified',worker)
        self.assertIn('relocated_engine_os_only_elf_verified',worker)
