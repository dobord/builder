...
def run(args,*,data=None,timeout=180,check=True,capture=False):
    ...
    if check and result.returncode: raise RuntimeError('Command failed: '+Path(args[0]).name+' code '+str(result.returncode))
    return result
def shell(code,**kw): return run(SSH+['bash','-se'],data=code.encode(),**kw)
def ps(code,**kw): return run(['pwsh','-NoProfile','-NonInteractive','-Command',"$ErrorActionPreference='Stop'; $ConfirmPreference='None'; "+code],**kw)
def download(url,path,expected=None,algorithm='sha256'):
    with urllib.request.urlopen(url,timeout=120) as response,path.open('wb') as output: shutil.copyfileobj(response,output,1024*1024)
    ...
    return digest
def screenshot(name):
    ps("Add-Type -AssemblyName System.Windows.Forms; Add-Type -AssemblyName System.Drawing; $r=[Windows.Forms.SystemInformation]::VirtualScreen; $b=New-Object Drawing.Bitmap($r.Width,$r.Height); $g=[Drawing.Graphics]::FromImage($b); $g.CopyFromScreen($r.Left,$r.Top,0,0,$b.Size); "+f"$b.Save('{ROOT/name}',[Drawing.Imaging.ImageFormat]::Png); $g.Dispose(); $b.Dispose()",check=False)
def windows():
...
def consent_fixture_dialogs(current,client_pid):
    if not any(w['pid']==client_pid and w['class']=='#32770' for w in current): return
    helper=Path(__file__).with_name('native-consent.ps1')
    run(['powershell.exe','-NoProfile','-NonInteractive','-Sta','-ExecutionPolicy','Bypass','-File',str(helper),'-ClientProcessId',str(client_pid),'-EvidenceDirectory',str(ROOT)],timeout=15)
    evidence=ROOT/'uia-dialog.json'
    if evidence.exists():
        value=json.loads(evidence.read_text(encoding='utf-8-sig'))
        REPORT.setdefault('fixture_consent_actions',[]).extend(value.get('actions') or [])
...
try:
    ...
    run([str(image_tool),'resize',str(disk),'32G']); run(['ssh-keygen','-q','-t','ed25519','-N','','-f',str(KEY)])
    ...
    ps(f"(Get-Item $env:WINDIR\\System32\\mstsc.exe).VersionInfo | Select-Object FileVersion,ProductVersion | ConvertTo-Json | Set-Content '{ROOT/'client-version.json'}'")
    (ROOT/'windows-before.json').write_text(json.dumps(windows()))
    client=subprocess.Popen([str(Path(os.environ['WINDIR'])/'System32'/'mstsc.exe'),str(rdp)],env=ENV); PROCS.append(client)
    seen=[]; stable_since=None
    for tick in range(60):
        time.sleep(2); current=windows(); seen.append({'seconds':2*(tick+1),'windows':current})
        consent_fixture_dialogs(current,client.pid)
        if tick in (1,4,9,19,29): screenshot(f'native-{tick}.png')
        matches=[w for w in current if w['pid']==client.pid and w['class'].startswith('RAIL') and ('FRDP xcalc' in w['title'] or w['title'].lower()=='xcalc')]
        if matches:
    ...
    REPORT['native_pass']=bool(REPORT.get('stable_window_seconds',0)>=10 and REPORT['exec_result_success'] and REPORT['frame_ack'])
except Exception as exc:
    REPORT['error_type']=type(exc).__name__; REPORT['error']=str(exc); traceback.print_exc(file=LOG)
finally:
    try:
        if KEY.exists():
            data=shell('sudo docker logs server 2>&1; sudo docker stop -t 5 server >/dev/null 2>&1 || true\n',capture=True,check=False,timeout=45).stdout; (ROOT/'server-final.log').write_bytes(data)
    ...
    for stem,stores in (('server',('LocalMachine\\Root',)),('signer',('CurrentUser\\My','LocalMachine\\Root','LocalMachine\\TrustedPublisher'))):
        try:
            file=ROOT/(stem+'-thumb.txt')
            if file.exists():
                thumb=file.read_text(encoding='utf-8-sig').strip()
                if len(thumb)==40 and all(c in '0123456789ABCDEFabcdef' for c in thumb):
                    for store in stores: ps(f"Remove-Item 'Cert:\\{store}\\{thumb}' -Force -Confirm:$false -ErrorAction SilentlyContinue",check=False)
        except Exception: pass
    try: credential('TERMSRV/localhost'); credential('TERMSRV/localhost:3390')
    except Exception: pass
